#!/usr/bin/perl
#
## Usage: perl ScoreComparison_Pos.pl
###########

use strict;
use warnings;

my $Refdir="casmi2017_neg";
my $preTol=0.05;
my $fraTol=0.05;
my %hashMatchedData=();
...
foreach my $cha (keys %hashPreMass)
{
	...
	my $rawFile="Negative/".$cha."-msms.mgf";
...
open(OUT,">NegativeScore0.05.txt") or die "Could not create the file:$!\n";
foreach my $k1 (sort (keys %hashMatchedData))
{
	...
	foreach my $k2 (keys %{$hashMatchedData{$k1}})
	{
		...
		my $H = HyperGeometric($N,$K,$N1,$K1); ## Recheck for $K and $K1
		my $rData=$hashRaw{$k1};
